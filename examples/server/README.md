...
1、注意：如果命令行参数和环境变量都为同一个参数设置，则参数将优先于环境变量。
*/
```
...
1、该项目包括一个基于 Web 的用户界面，可通过“/chat/completions”端点与模型进行交互
2、这个项目包含：用户界面（这个用户界面可以通过上述提到的endpoint来同模型进行交互）
*/
```
...
1、这个web ui的开发使用到了三个工具：vue、tailwindcss、daisyui、vite
2、前端开发用到了vue框架
...
1、注意：如果您使用的是 vite dev 服务器，则可以将 API 基本 URL 更改为 llama.cpp。为此，请在浏览器的控制台中运行此代码片段：
*/
```
...
1、确保模型的路径是正确的，这样可以使用llama-server
*/
```
...
### 9.1 GET `/health`: Returns heath check result
...
1、这部分内容介绍的是对模型的设置以及如何获取得到模型的输出
2、下列涉及到的一些参数我都是不理解的
*/
```
*Options:*
...
### 9.3 POST `/tokenize`: Tokenize a given text
...
### 9.4 POST `/detokenize`: Convert tokens to text

*Options:*

`tokens`: Set the tokens to detokenize.

### 9.5 POST `/embedding`: Generate embedding of a given text
...
### 9.6 POST `/reranking`: Rerank documents according to a given query
...
### 9.7 POST `/infill`: For code infilling.
...
### 9.8 **GET** `/props`: Get server global properties.
...
### 9.9 POST `/props`: Change server global properties.
...
### 9.10 POST `/v1/chat/completions`: OpenAI-compatible Chat Completions API
```c
/*
notes:杨小兵-2024-12-17

1、这部分内容介绍的是兼容OpenAI聊天的API
2、我的一个问题是是否兼容OpenAI API
*/
```
...
1、查看上述提到的有关openai的文档可以获取一些帮助
2、openai文档规范中的一些特性例如function calling不会被支持，llama.cpp项目中的/completion规范特性例如mirostat则是被支持的
*/
```
...
### 9.11 POST `/v1/embeddings`: OpenAI-compatible embeddings API
...
### 9.12 GET `/slots`: Returns the current slots processing state
...
### 9.13 GET `/metrics`: Prometheus compatible metrics exporter
...
### 9.14 POST `/slots/{id_slot}?action=save`: Save the prompt cache of the specified slot to a file.
...
### 9.15 POST `/slots/{id_slot}?action=restore`: Restore the prompt cache of the specified slot from a file.
...
### 9.16 POST `/slots/{id_slot}?action=erase`: Erase the prompt cache of the specified slot.
...
### 9.17 GET `/lora-adapters`: Get list of all LoRA adapters
...
### 9.18 POST `/lora-adapters`: Set list of LoRA adapters
...
1、扩建或建立替代Web前端
2、如果你想要扩展前端，那么可以通过添加参数--path运行该服务程序,将--path设置为自定义目录并且在这个目录中导入completion.js并且获取llamaComplete方法
*/
```
