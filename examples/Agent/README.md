...
A C++ agent that manages llama-server and provides tool calling capabilities for LLaMA models.
...
1. Place the Agent folder in `llama.cpp/examples/`
...
2. Agent forwards request to llama-server with tool definitions
3. Model returns tool call request
4. Agent executes the requested tool
5. Agent sends tool results back to model
6. Model generates final response with tool results
7. Agent returns complete response to user
