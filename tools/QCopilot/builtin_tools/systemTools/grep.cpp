...
constexpr const char* k_tool_name = "grep";
...
json run_grep(const json& args) {
    ...
    // 参数校验
    if (pat.empty()) {
        json err = common::make_error(k_tool_name, "pattern is required");
        err["messages"] = json::array({"Missing 'pattern' argument"});
        return err;
    }
    std::string error_message;
    if (!common::validate_existing_path(target_path, error_message)) {
    ...
    if (!common::file_exists(target_path)) {
        LOG_ERR("grep: Path not found: %s", target_path.c_str());
        json err = common::make_error(k_tool_name, "Path not found: " + target_path);
        err["path"] = target_path;
        err["pattern"] = pat;
    ...
    try {
        fs::path p = common::utf8_to_path(target_path);
        std::error_code ec;

        if (fs::is_regular_file(p, ec)) {
            // 直接在单个文件中搜索
            auto per_file = common::search_in_file_regex(
            ...
            // 仅收集“文件”，不包含目录
            std::vector<fs::path> files = common::glob_paths(
            ...
            json err = common::make_error(k_tool_name, "Unsupported path type for grep: " + target_path);
            err["path"] = target_path;
            err["pattern"] = pat;
        ...
        json err = common::make_error(k_tool_name, "Failed to execute grep: " + std::string(e.what()));
        err["path"] = target_path;
        err["pattern"] = pat;
