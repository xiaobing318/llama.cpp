...
int main(){
    ...
    auto r1 = BuiltinTools::FileTools::executeValidateUtf8File({{"path", ok.u8string()}});
    ...
    auto r2 = BuiltinTools::FileTools::executeValidateUtf8File({{"path", bad.u8string()}});
    T.check(r2.value("success", false) == true, "validate_utf8_file JSON success (bad)");
    T.check(r2.value("is_utf8", true) == false, "is_utf8 false");

    // 错误：目录路径
    auto r3 = BuiltinTools::FileTools::executeValidateUtf8File({{"path", root.u8string()}});
