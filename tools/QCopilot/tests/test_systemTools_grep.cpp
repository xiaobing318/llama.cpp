...
int main(){
    ...
    BuiltinTools::FileTools::executeWriteTextFile({{"path", (root/"a.txt").u8string()}, {"content","Hello\nWorld\n"}});
    BuiltinTools::FileTools::executeWriteTextFile({{"path", (root/"b.txt").u8string()}, {"content","alpha\nbeta\n"}});

    // 单文件搜索
    auto r1 = BuiltinTools::SystemTools::executeGrep({{"path", (root/"a.txt").u8string()}, {"pattern","Hello"}, {"line_numbers", true}});
    ...
    // 目录递归 + file_glob
    auto r2 = BuiltinTools::SystemTools::executeGrep({
        {"path", root.u8string()},
        {"pattern", "beta"},
        {"recursive", true},
        {"file_glob", "**/*.txt"},
    ...
    // 正则
    auto r3 = BuiltinTools::SystemTools::executeGrep({{"path", (root/"a.txt").u8string()}, {"pattern","^W.*d$"}, {"use_regex", true}});
