...
int main() {
    ...
    T.check(!validatePath(std::string(5000, 'a'), err),
            "针对超过 4096 字节路径的测试没有返回 false，说明接口存在问题。");
    err.clear();
    // 测试用例：针对 URL 形式路径的测试应该返回 false
    T.check(!validatePath("https://示例.com/资源.txt", err),
    ...
    T.check(validatePath(windowsStyleFile, err),
    ...
    T.check(!validatePath(trailingSpace, err),
    ...
    T.check(!validatePath("C:\\临时\\不存在.txt", err),
